function parser(tokens) {
    ...
    function parsePrimary() {
        ...
        if (token.type === "SUPER") {
            ...
            if (tokens[i + 1] && tokens[i + 1].type === "LEFT_PAREN") {
                throw new Error('"super()" ora bisa digunakake minangka ekspresi');
            }
            i++;
            return {
                ...
                while (tokens[i] && (tokens[i].type === "LEFT_BRACKET" || tokens[i].type === "DOT")) {
                    if (tokens[i].type === "LEFT_BRACKET") {
                        ...
                        if (!tokens[i] || tokens[i].type !== "IDENTIFIER") {
                            throw new Error('Dibutuhake jeneng properti sawise "." (Dibutuhkan nama properti setelah ".")');
                        }
    ...
    function parseStatement() {
            ...
            if (tokens[i + 1] && (tokens[i + 1].type === "DOT" || tokens[i + 1].type === "LEFT_BRACKET" || tokens[i + 1].type === "LEFT_PAREN")) {
                ...
                if (tokens[look] && tokens[look].type === "EQUALS") {
                    ...
                    if (target.type !== "IndexExpression") {
                        throw new Error('Target assignment ora valid');
                    }
                    if (!tokens[i] || tokens[i].type !== "EQUALS") {
                        throw new Error('Sawise akses properti kudu ana tanda "=" kanggo assignment');
                    }
                    i++; // lewati "="
                    if (i >= tokens.length) {
                        throw new Error('Dibutuhake nilai sawise "="');
        ...
        if (token.type === "SUPER") {
            ...
            if (tokens[i + 1] && tokens[i + 1].type === "EQUALS") {
                throw new Error('"super" ora bisa di-assign langsung');
            }

            // Constructor call: super(args...)
            if (tokens[i + 1] && tokens[i + 1].type === "LEFT_PAREN") {
                if (!inConstructor) {
                    throw new Error('"super()" mung bisa digunakake ing njero constructor "wiwiti"');
                }
                i++; // lewati "super"
                i++; // lewati "("
                ...
                if (!tokens[i] || tokens[i].type !== "RIGHT_PAREN") {
                    throw new Error('"super()" mbutuhake kurung tutup ")" sawise argumen');
                }
                i++; // lewati ")"
            ...
            if (tokens[i + 1] && (tokens[i + 1].type === "DOT" || tokens[i + 1].type === "LEFT_BRACKET")) {
                ...
                if (tokens[look] && tokens[look].type === "EQUALS") {
                    throw new Error('Property "super" ora bisa di-assign langsung');
                }
            ...
            throw new Error('"super" kudu diterusake nganggo "(" kanggo constructor utawa "."/"[" kanggo method');
        }
        ...
        if (token.type === "BENTUK") {
            ...
            if (tokens[i] && tokens[i].type === "NGEMBANGAKE") {
                ...
                if (!tokens[i] || tokens[i].type !== "IDENTIFIER") {
                    throw new Error('Sawise "ngembangake" kudu ana jeneng struct induk');
                }
